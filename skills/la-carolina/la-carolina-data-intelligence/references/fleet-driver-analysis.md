...
## Múltiples instancias Supabase

Cuando el usuario proporcione credenciales de una **nueva instancia Supabase**:

1. Explorar esquema primero: probar 20+ nombres de tabla comunes
2. Reportar qué tablas existen y su estructura (columnas, conteo)
3. Determinar rango de fechas (año más antiguo → más reciente)
4. Indicar si sirve o no para el análisis solicitado

Ejemplo real: instancia `lqeddrpbwunzcyjxuiei.supabase.co` solo tenía `conductores` (168 registros, todos activos, sin placa asignada) e `incentivos` vacío — no servía para comparativa anual.

## Per-bus driver concentration

Cada bus tiene **1-2 conductores principales** que hacen **90%+ de los viajes**. Los demás son relevos esporádicos. Esto es importante al reportar números de conductores únicos — la mayoría son relevos de 1-3 viajes.

Ver `references/per-bus-driver-concentration.md` para detalle.
